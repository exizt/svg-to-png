class SvgToPng {
    ...
    private _canvasContainerId:string = ''
	 ...
	 */
    loadPresetWithDraw(presets:IPreset[]){
        // console.log(presets)
	...
	*/
	createWithDrawCanvas(presetId:string){
	...
	*/
	createCanvasElement(size:number){
	...
	*/
	drawCanvas2(canvas:HTMLCanvasElement){
	...
	*/
	downloadPng(){
		...
		function download(canvasId:string, size:number){
			...
			link.href = (document.getElementById(canvasId) as HTMLCanvasElement).toDataURL("image/png")
			link.click();
         ...
         */
        function formatUnicorn(rule:string, args:any) {
            var str = rule;
